...
public class RequestInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (logRequest) {
            ...
            String[] ps1 = request.url().toString().split("\\?");
...
//                            e1.printStackTrace();
            ...
            //打印请求信息
            Timber.d("[" + request.method() + "]:" + request.url().url().toString().split("\\?")[0]
                    + "\n请求头：\n" + request.headers() + "\n上送参数:" + map);
        }
...
//        }
        ...
        //打印响应结果
        String bodyString = printResult(request, originalResponse.newBuilder().build(), logResponse, t1, t2);
     ...
     */
    @Nullable
    private String printResult(Request request, Response response, boolean logResponse, long t1, long t2) throws IOException {
        //读取服务器返回的结果
        ResponseBody responseBody = response.body();
        StringBuffer sb = new StringBuffer();
        String logs = "";
        sb.append("响应的服务器路径:" + request.url().url().toString().split("\\?")[0] + "[" + TimeUnit.NANOSECONDS.toMillis(t2 - t1) + " ms]"
                + "\n" + "响应头:\n" + response.headers()
                + "\n");
        ...
        if (isParseable(mediaType)) {
            ...
            if (logResponse) {
                logs = isJson(mediaType) ?
                        CharacterHandler.jsonFormat(bodyString) : isXml(mediaType) ?
        ...
        if (logResponse) {
            Timber.d(sb.toString() + "响应信息格式化后数据如下框:");
            Logger.json(logs);
//            if (mediaType != null && mediaType.toString().toLowerCase().contains("text")) {
//                Timber.d(sb.append("响应信息:" + logs).toString());
//            } else {
//                Timber.d(sb.toString() + "响应信息格式化后数据如下框:");
//                if (isJson(mediaType)) {
//                    Logger.json(logs);
...
//            }
    ...
    private ContentType getRequestType(MediaType mediaType) {
        return isParseable(mediaType) ? ContentType.text : isJson(mediaType) ? ContentType.json
                : isXml(mediaType) ? ContentType.xml : isHtml(mediaType) ? ContentType.html : isForm(mediaType) ? ContentType.form : null;
    }
